package com.ping.service.controller;
...
import com.ping.service.entity.User;
import com.ping.service.service.UserService;
import com.ping.service.vo.ResponseVO;
...
public class UserController {
    ...
    @GetMapping("/one")
    public User getByUserId(@ApiParam(value = "用户ID", required = true) Long user_id) {
        return this.userService.findByPk(new User(user_id));
    }

    @ApiOperation(value = "查询所有用户", notes = "查询条件:user_name like, user_account like")
    @GetMapping("/all")
    public List<User> listAll(@ApiParam(value = "用户信息") User user) {
        ...
        return this.userService.find(cons);
    }
